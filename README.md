...
SQLite 是应用的主数据库，文件持久化在 `./data/marathon.sqlite`。所有页面读取和打卡写入都只依赖本机 SQLite，因此 Notion 网络故障不会影响应用使用。

每次打卡会更新 SQLite 并写入同步队列；后台每分钟向 Notion 推送，成功后删除队列项。后台每 10 分钟从 Notion 拉取一次外部修改，但不会覆盖仍在等待同步的本地记录。旧版 `workouts-cache.json` 会在首次启动时自动迁移。
